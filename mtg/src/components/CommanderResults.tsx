import type { CommanderMatch } from '../types/commander'
...
export function CommanderResults({ matches, loading, emptyMessage }: Props) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20 text-[var(--color-mtg-muted)]">
        <span className="animate-pulse">Matching commanders…</span>
      </div>
    )
  }
  ...
  if (matches.length === 0) {
    return (
      <p className="py-12 text-center text-[var(--color-mtg-muted)]">
        {emptyMessage ?? 'No commanders matched.'}
      </p>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {matches.map((m) => {
        const img = m.commander.image
        return (
          <button
            key={m.commander.id}
            type="button"
            onClick={() => openDetail(commanderToDetail(m.commander))}
          ...
          >
            {img ? (
              <img
                src={img}
                alt={m.commander.name}
                className="h-36 w-24 shrink-0 rounded object-cover"
                loading="lazy"
              />
            ) : (
              <div className="flex h-36 w-24 shrink-0 items-center justify-center rounded bg-[var(--color-mtg-bg)] text-xs text-center p-1">
                {m.commander.name}
              </div>
            )}
            <div className="min-w-0 flex-1">
              <div className="flex items-start justify-between gap-2">
                <p className="font-semibold leading-tight">{m.commander.name}</p>
                <div className="flex shrink-0 flex-col items-end gap-1">
                  ...
                  <EdhrecBadge name={m.commander.name} rank={m.commander.edhrec_rank} />
                </div>
              </div>
              {showPopularity && m.commander.edhrec_rank && (
                <p className="mt-0.5 text-[10px] text-emerald-400/80">
                  {popularityLabel(m.commander.edhrec_rank)}
                </p>
              )}
              <p className="mt-0.5 text-xs text-[var(--color-mtg-muted)]">
                {m.commander.type_line}
              </p>
              ...
              <p className="mt-2 line-clamp-3 text-[11px] text-[var(--color-mtg-muted)]">
                {m.commander.oracle_text.slice(0, 200)}
                {m.commander.oracle_text.length > 200 ? '…' : ''}
              </p>
            </div>
          </button>
        )
